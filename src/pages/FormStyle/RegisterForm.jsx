/**
 * Register Form editor (Pro-only — wrapped by ProLock in FormStyle.jsx).
 */
...
const RegisterForm = () => {
	const { settings, isSaving, isPro } = useSelect( ( select ) => {
		...
		return {
			settings: store.getSectionSettings( 'register' ),
	...
	const [ previewUrl ] = useState( () => {
		...
		return `${ base }admin-ajax.php?action=wpepp_preview&type=register&_wpnonce=${ window.wpeppData?.previewNonce || '' }`;
	} );
	...
	const update = useCallback( ( key, value ) => {
		updateSetting( 'register', key, value );
	}, [ updateSetting ] );

	const handleSave = useCallback( () => {
		saveSettings( 'register', s );
	}, [ saveSettings, s ] );

	const handleReset = useCallback( () => {
		resetSettings( 'register' );
	}, [ resetSettings ] );
						...
						<>
							<PanelRow>
								<TextControl
									...
									placeholder={ __( 'Register', 'wp-edit-password-protected' ) }
								/>
							</PanelRow>
							<PanelRow>
								<ColorControl
				...
				<PanelBody title={ __( 'Custom CSS', 'wp-edit-password-protected' ) } initialOpen={ false }>
					<PanelRow>
						<TextareaControl
							...
							help={ __( 'Add your custom CSS for the register page.', 'wp-edit-password-protected' ) }
							value={ s.custom_css || '' }
