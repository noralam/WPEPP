/**
 * Lost Password Form editor (Pro-only — wrapped by ProLock in FormStyle.jsx).
 */
...
const LostPasswordForm = () => {
	const { settings, isSaving, isPro } = useSelect( ( select ) => {
		...
		return {
			settings: store.getSectionSettings( 'lostpassword' ),
	...
	const [ previewUrl ] = useState( () => {
		...
		return `${ base }admin-ajax.php?action=wpepp_preview&type=lostpassword&_wpnonce=${ window.wpeppData?.previewNonce || '' }`;
	} );
	...
	const update = useCallback( ( key, value ) => {
		updateSetting( 'lostpassword', key, value );
	}, [ updateSetting ] );

	const handleSave = useCallback( () => {
		saveSettings( 'lostpassword', s );
	}, [ saveSettings, s ] );

	const handleReset = useCallback( () => {
		resetSettings( 'lostpassword' );
	}, [ resetSettings ] );
	...
	return (
		<div className="wpepp-editor-layout">
				...
				<PanelBody title={ __( 'Page Background', 'wp-edit-password-protected' ) } initialOpen={ false }>
					<PanelRow>
						<SelectControl
							...
							onChange={ ( v ) => update( 'page.background_type', v ) }
						/>
					</PanelRow>
						...
						<>
							<PanelRow>
								<TextControl
									...
									placeholder={ __( 'Lost Password', 'wp-edit-password-protected' ) }
								/>
							</PanelRow>
							<PanelRow>
								<ColorControl
				...
				<PanelBody title={ __( 'Custom CSS', 'wp-edit-password-protected' ) } initialOpen={ false }>
					<PanelRow>
						<TextareaControl
							...
							help={ __( 'Add your custom CSS for the lost password page.', 'wp-edit-password-protected' ) }
							value={ s.custom_css || '' }
			...
			<LivePreview previewUrl={ previewUrl } css={ css } />
		</div>
	);
};
